...
""")
    ...
    @Query(
            value = """
    SELECT e FROM Equipment e
    WHERE (:name IS NULL OR LOWER(e.equipmentName) LIKE LOWER(CONCAT('%', :name, '%')))
    AND (:location IS NULL OR LOWER(e.location) = LOWER(:location))
    AND (:minPrice IS NULL OR e.priceRate IS NOT NULL AND e.priceRate >= :minPrice)
    AND (:maxPrice IS NULL OR e.priceRate IS NOT NULL AND e.priceRate <= :maxPrice)
    AND (:minRating IS NULL OR e.serviceRating IS NOT NULL AND e.serviceRating >= :minRating)
    """,
            countQuery = """
